...
def get_item_info(input_file):
    ...
    for line in fp:
        ...
        if len(item) > 3:
            ...
            itemId = item[0]
            genre = item[-1]
            #there may be comma in the title
            title = ",".join(item[1:-1])
...
def get_ave_score(input_file):
    ...
    fp.close()
    for itemid,record in record_dict:
        score_dict[itemid] = round(record[1]/record[0],3)
    return score_dict

def get_train_data(input_file):
    """
    get train data for lfm which is the matrix of user-score
    Args:
        input_file: user item ratings
    Returns:
        a list:[(user_id,itemid,label),...]
    """
    if not os.path.exists(input_file):
        return []
    ...
    fp.close()
    # negative sampling and balance the datasets
    # here we use 1 or 0 to mean like or not like rather than specific socre
    # we plan to get the TopN recommendation rather than exact score
    for userid in pos_dict:
        data_num = min(len(pos_dict),len(neg_dict.get(userid,[])))
        if data_num > 0:
